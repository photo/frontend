...
".json",c=b.parent(),b={crumb:g.get()};OP.Util.makeRequest(a,b,function(a){204===a.code?c.slideUp("medium",function(){console.log(this);$(this).remove()}):opTheme.message.error("We could not delete this group.")})},albumForm:function(a){a.preventDefault();a=$(a.target).attr("href")+".json";OP.Util.makeRequest(a,{modal:"true",dynamic:"true"},function(a){if(200===a.code){var c=$("#modal"),a=h.modal("Create an album",a.result.markup,null);c.html(a).modal();$(".typeahead",c).chosen()}else opTheme.message.error("Could not load the form to create an album.")},
...
opTheme.message.confirm("Your features were successfully saved."):opTheme.message.error("We could not save your features.")},groupDelete:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+".json",c=b.parent(),b={crumb:g.get()};OP.Util.makeRequest(a,b,function(a){204===a.code?c.slideUp("medium"):opTheme.message.error("We could not delete this group.")})},groupEmailAdd:function(a){a.preventDefault();var a=$(a.target).prev(),b=$("ul.group-emails-add-list",a.parent()),c=a.val();""!==c&&
($('<li><span class="group-email-queue">'+c+'</span> <a href="#" class="group-email-remove-click"><i class="group-email-remove-click icon-minus-sign"></i></a></li>').prependTo(b),a.val(""))},groupEmailRemove:function(a){a.preventDefault();$(a.target).parent().parent().remove()},groupForm:function(a){a.preventDefault();a=$(a.target).attr("href")+".json";OP.Util.makeRequest(a,{modal:"true",dynamic:"true"},function(a){if(200===a.code){var c=$("#modal"),a=h.modal("Create a group",a.result.markup,null);
c.html(a).modal()}else opTheme.message.error("Could not load the form to create an album.")},"json","get");return!1},groupPost:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c=-1<b.search("create"),d=$('input[name="dynamic"]',a).val(),e,f={name:$('input[name="name"]',a).val()};f.crumb=g.get();$(".group-email-add-click",a).trigger("click");e=[];$("span.group-email-queue",a).each(function(a,b){e.push($(b).html())});f.members=e.join(",");OP.Util.makeRequest(b,f,function(a){var b;
...
$(a.target).parent().parent().slideUp("fast",function(){$(this).remove()})},modalUnload:function(a){a.preventDefault();$("#modal-photo-detail").html("");f.replace(l)},passwordRequest:function(a){a.preventDefault();a=$(a.target).parent();a=$("#login-email",a).val();OP.Util.makeRequest("/user/password/request.json",{email:a},opTheme.callback.passwordRequestCb)},passwordRequestCb:function(a){$("#loginBox").modal("hide");a.result?opTheme.message.confirm("An email has been sent with a link to reset your password."):
...
return!1},photoEdit:function(a){a.preventDefault();a="/photo/"+$(a.target).attr("data-id")+"/edit.json";OP.Util.makeRequest(a,{},function(a){if(200===a.code){var c=$("#modal"),a=h.modal("Edit this photo",a.result.markup,null);c.html(a).modal();$(".typeahead",c).chosen()}else opTheme.message.error("Could not load the form to edit this photo.")},"json","get");return!1},photoView:function(a){a.preventDefault();a=$(a.target).parent();$(".photo-view");var b=a.attr("href"),a=b;if($("body").hasClass("photo-details"))location.href=
b;else{var c=$("#modal-photo-detail");$("#modal-photo-detail .photo-view");a=-1===a.indexOf("?")?a+"?modal=true":a+"&modal=true";c.load(a+" .photo-view",function(a){$("img.next-photo");$("img.previous-photo");f.insert(b,f.parse(a));k.fetchAndCacheNextPrevious()})}return!1},photoViewModal:function(a){a.preventDefault();a=$(a.target).parent();$(".photo-view");var b=a=a.attr("href"),c=$("#modal-photo-detail"),b=-1===b.indexOf("?")?b+"?modal=true":b+"&modal=true";location.hash=a;c.load(b+" .photo-view",
opTheme.callback.photoViewModalCb).modal().on("hidden",opTheme.callback.modalUnload);return!1},photoViewModalCb:function(a){k.fetchAndCacheNextPrevious();f.insert(location.hash,f.parse(a))},photoUpdate:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c=a.serialize();0===$('select[name^="groups"] option:checked',a).length&&(c+="&groups=");0===$('select[name^="albums"] option:checked',a).length&&(c+="&albums=");OP.Util.makeRequest(b,c,opTheme.callback.photoUpdateCb,"json",
...
return!1}},init:{load:function(a){var b="state"in window.history,c=location.href;g.set(a);OP.Tag.init();l=location.pathname;History.Adapter.bind(window,"statechange",function(){var a=History.getState(),e=!b&&location.href==c;b=!0;e||f.render(a.data)});$(".dropdown-toggle").dropdown();"/"===location.pathname?opTheme.init.pages.front():"/manage"===location.pathname?opTheme.init.pages.manage():0===location.pathname.search(/^\/photos(.*)\/list/)?opTheme.init.pages.photos.init():0===location.pathname.search(/^\/p\/[a-z0-9]+/)||
...
a.result,b=opTheme.init.pages.photos,c=$(".infobar"),d=$(".startdate",c),c=$(".enddate",c);parseInt(d.attr("data-time"));var e=parseInt(c.attr("data-time")),f=opTheme.ui,h;if(a[0].totalPages>=b.page){var g;for(h=0;h<a.length;h++)g=parseInt(a[h].dateTaken),g>e?(f.fadeAndSet(c,phpjs.date("l F jS, Y",g)),c.attr("data-time",g),e=g):parseInt(a[h].dateTaken)<parseInt(d.attr("data-time"))&&(f.fadeAndSet(d,phpjs.date("l F jS, Y",g)),d.attr("data-time",g));Gallery.showImages($(".photo-grid-justify"),a);b.page++;
