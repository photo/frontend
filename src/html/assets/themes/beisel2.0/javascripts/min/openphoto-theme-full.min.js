...
var b=$(a.target),a=b.attr("href")+".json",c={crumb:f.get()};OP.Util.makeRequest(a,c,function(a){204===a.code?(b.parent().parent().slideUp("medium",function(){this.remove()}),opTheme.message.confirm("Application successfully deleted.")):opTheme.message.error("Could not delete Application.")});return!1},featuresPost:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c={},c={crumb:f.get()};c.allowDuplicate=$('input[name="allowDuplicate"]:checked',a).length;c.downloadOriginal=
$('input[name="downloadOriginal"]:checked',a).length;c.hideFromSearchEngines=$('input[name="hideFromSearchEngines"]:checked',a).length;OP.Util.makeRequest(b,c,opTheme.callback.featuresPostCb)},featuresPostCb:function(a){200===a.code?opTheme.message.confirm("Your features were successfully saved."):opTheme.message.error("We could not save your features.")},groupDelete:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+".json",c=b.parent(),b={crumb:f.get()};OP.Util.makeRequest(a,b,function(a){204===
...
$('input[name="dynamic"]',a).val(),h,e={name:$('input[name="name"]',a).val()};e.crumb=f.get();$(".group-email-add-click",a).trigger("click");h=[];$("span.group-email-queue",a).each(function(a,b){h.push($(b).html())});e.members=h.join(",");OP.Util.makeRequest(b,e,function(a){var b;200===a.code||201===a.code?d?opTheme.callback.groupPostDynamicCb(b,a.result):c?window.location.href="/manage/groups?m=group-created&rnd="+Math.random()+"#group-"+a.result.id:opTheme.message.confirm("Group updated successfully."):
...
$(this).attr("data-id");container=$(this).parent();container.hasClass("pinned")||OP.Batch.add(id)})},pluginView:function(a){a.preventDefault();var a=$(a.target).attr("href")+".json",b={crumb:f.get()},c=a.match(/\/plugin\/(.*)\/view.json$/);OP.Util.makeRequest("/plugin/"+c[1]+"/view.json",b,function(a){200===a.code?$("#modal").html(e.modal("Update "+c[1],a.result.markup,null)).modal():opTheme.message.error("Unable to load this plugin for editing.")},"json","get")},pluginStatusToggle:function(a){a.preventDefault();
var b=$(a.target),a=b.attr("href")+".json",c={crumb:f.get()};OP.Util.makeRequest(a,c,function(a){var c=$(b).parent(),e=c.parent();200===a.code?($("div",e).removeClass("hide"),c.addClass("hide")):opTheme.message.error("Could not update the status of this plugin.")},"json","post");return!1},pluginUpdate:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json";OP.Util.makeRequest(b,a.serializeArray(),function(a){200===a.code?opTheme.message.confirm("Your plugin was successfully updated."):
...
var a=$(a.target),b={method:"feed",display:"popup"};b.link=a.attr("data-link");b.picture=a.attr("data-picture");b.name=a.attr("data-name");b.description=a.attr("data-description");FB.ui(b,function(){window.close()})}},tagsInitialized:function(){var a=OP.Tag.getTags(),b="";if(null!==a&&0<a.length)for(i in a)b+='<option value="'+a[i]+'">'+a[i]+"</option>";$(".typeahead-tags").html(b).chosen()},uploadCompleteSuccess:function(a){$("form.upload").fadeOut("fast",function(){OP.Util.makeRequest("/photos/upload/confirm.json",
...
opTheme.message.confirm("Credential successfully deleted.")):opTheme.message.error("Could not delete credential.")});return!1}},init:{load:function(a){var b="state"in window.history&&null!==window.history.state,c=location.href;f.set(a);OP.Tag.init();l=location.pathname;History.Adapter.bind(window,"statechange",function(){var a=History.getState(),h=!b&&location.href==c;b=!0;h||g.render(a.data)});$(".dropdown-toggle").dropdown();$(".modal").on("shown",opTheme.callback.modalShown);"/"===location.pathname?
