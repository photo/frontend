...
function(a){204===a.code?$(".action-container-"+c).hide("medium",function(){$(this).remove()}):opTheme.message.error("Could not delete the photo.")});return!1},actionPost:function(a){a.preventDefault();var b=$("#favorite-form"),a=b.attr("action")+".json",b=b.serialize();$.ajax({url:a,type:"POST",data:b,dataType:"json",success:function(a){location.href=201===a.code?location.pathname+"?c=favorited":location.pathname+"?e=unknown"}})},albumDelete:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+
".json",c=b.parent(),b={crumb:j.get()};OP.Util.makeRequest(a,b,function(a){204===a.code?c.slideUp("medium",function(){this.remove()}):opTheme.message.error("We could not delete this group.")})},albumForm:function(a){a.preventDefault();a=$(a.target).attr("href")+".json";OP.Util.makeRequest(a,{modal:"true",dynamic:"true"},function(a){if(200===a.code){var c=$("#modal"),a=e.modal("Create an album",a.result.markup,null);c.html(a).modal();$(".typeahead",c).chosen()}else opTheme.message.error("Could not load the form to create an album.")},
"json","get");return!1},albumPost:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c=-1<b.search("create"),d=parseInt($('input[name="dynamic"]',a).val()),g=$("select[name='groups']",a).val(),h={};h.name=$('input[name="name"]',a).val();h.permission=$('input[name="permission"]:checked',a).val();h.groups=null!==g?g.join(","):"";h.crumb=j.get();0==h.name.length?opTheme.message.error("Please enter a name for your album."):OP.Util.makeRequest(b,h,function(a){var b;200===a.code||
201===a.code?d?opTheme.callback.albumPostDynamicCb(b,a.result):c?location.href="/manage/albums?m=album-created#album-"+a.result.id:opTheme.message.confirm("Album updated successfully."):opTheme.message.error("Could not update album.")})},albumPostDynamicCb:function(a,b){var c=$('select[name="albums"]',a);$(".modal").modal("hide");$('<option value="'+b.id+'" selected="selected">'+b.name+"</option>").prependTo(c);c.trigger("liszt:updated")},albumShowAll:function(a){a.preventDefault();var a=$(".album-list"),
b=a.outerHeight(),c=opTheme.init.pages.photos.albumContainerHeight[1],d=opTheme.init.pages.photos.albumContainerHeight[0],g={height:0};g.height=b==c?d+"px":c+"px";a.animate(g)},batchAdd:function(a){$(".pin.photo-"+a.id).addClass("revealed pinned");opTheme.ui.batchMessage();k("Adding photo "+a.id)},batchClear:function(){$(".pin.pinned").removeClass("revealed pinned");opTheme.ui.batchMessage()},batchField:function(a){var a=$(a.target).val(),b=$("form#batch-edit .form-fields");switch(a){case "albumsAdd":b.html(opTheme.ui.batchFormFields.albums());
$("select",b).chosen();break;case "delete":b.html(opTheme.ui.batchFormFields.empty());break;case "groups":b.html(opTheme.ui.batchFormFields.groups());$("select",b).chosen();break;case "permission":b.html(opTheme.ui.batchFormFields.permission());break;case "tagsAdd":case "tagsRemove":b.html(opTheme.ui.batchFormFields.tags())}},batchModal:function(){var a=$("#modal"),b=e.modal("Batch edit your pinned photos",'<form id="batch-edit">  <div class="clearfix">    <label>What would you like to do?</label>    <div class="input">      <select id="batch-key" class="batch-field-change" name="property">        <option value="tagsAdd">Add Tags</option>        <option value="tagsRemove">Remove Tags</option>        <option value="albumsAdd">Add Albums</option>        <option value="groups">Update Groups</option>        <option value="permission">Update Permissions</option>        <option value="delete">Delete</option>      </select>    </div>  </div>  <div class="form-fields">'+
opTheme.ui.batchFormFields.tags()+"</div></form>",'<a href="#" class="btn photo-update-batch-click">Submit</a>');a.html(b).modal()},batchRemove:function(a){$(".pin.photo-"+a).removeClass("pinned revealed");opTheme.ui.batchMessage();k("Removing photo "+a)},commentJump:function(a){a.preventDefault();$.scrollTo($("div.comment-form"),200);return!1},credentailDelete:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+".json";OP.Util.makeRequest(a,{},function(a){204===a.code?(b.parent().parent().slideUp("medium",
function(){this.remove()}),opTheme.message.confirm("Application successfully deleted.")):opTheme.message.error("Could not delete Application.")});return!1},featuresPost:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c={},c={crumb:j.get()};c.allowDuplicate=$('input[name="allowDuplicate"]:checked',a).length;c.downloadOriginal=$('input[name="downloadOriginal"]:checked',a).length;OP.Util.makeRequest(b,c,opTheme.callback.featuresPostCb)},featuresPostCb:function(a){200===a.code?
opTheme.message.confirm("Your features were successfully saved."):opTheme.message.error("We could not save your features.")},groupDelete:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+".json",c=b.parent(),b={crumb:j.get()};OP.Util.makeRequest(a,b,function(a){204===a.code?c.slideUp("medium",function(){this.remove()}):opTheme.message.error("We could not delete this group.")})},groupEmailAdd:function(a){a.preventDefault();var a=$(a.target).prev(),b=$("ul.group-emails-add-list",a.parent()),
c=a.val();""!==c&&($('<li><span class="group-email-queue">'+c+'</span> <a href="#" class="group-email-remove-click"><i class="group-email-remove-click icon-minus-sign"></i></a></li>').prependTo(b),a.val(""))},groupEmailRemove:function(a){a.preventDefault();$(a.target).parent().parent().remove()},groupForm:function(a){a.preventDefault();a=$(a.target).attr("href")+".json";OP.Util.makeRequest(a,{modal:"true",dynamic:"true"},function(a){if(200===a.code){var c=$("#modal"),a=e.modal("Create a group",a.result.markup,
null);c.html(a).modal()}else opTheme.message.error("Could not load the form to create an album.")},"json","get");return!1},groupPost:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c=-1<b.search("create"),d=$('input[name="dynamic"]',a).val(),g,h={name:$('input[name="name"]',a).val()};h.crumb=j.get();$(".group-email-add-click",a).trigger("click");g=[];$("span.group-email-queue",a).each(function(a,b){g.push($(b).html())});h.members=g.join(",");OP.Util.makeRequest(b,h,function(a){var b;
...
$(a.target).parent().parent().slideUp("fast",function(){$(this).remove()})},modalUnload:function(a){a.preventDefault();$("#modal-photo-detail").html("");f.replace(m)},passwordRequest:function(a){a.preventDefault();a=$(a.target).parent();a=$("#login-email",a).val();OP.Util.makeRequest("/user/password/request.json",{email:a},opTheme.callback.passwordRequestCb)},passwordRequestCb:function(a){$("#loginBox").modal("hide");a.result?opTheme.message.confirm("An email has been sent with a link to reset your password."):
opTheme.message.error("We were unable to send you a password request. Make sure you entered your email address correctly and that you are the owner of this site.")},passwordReset:function(a){a.preventDefault();var a=$(a.target).parent(),b=a.serializeArray();$(".input-password",a).val()!=$(".input-password-confirm",a).val()?opTheme.message.error("Your passwords did not match."):OP.Util.makeRequest("/user/password/reset.json",b,opTheme.callback.passwordResetCb)},passwordResetCb:function(a){a.result?
opTheme.message.confirm("Your password was updated successfully. You can now log in to your site."):opTheme.message.error("We were unable to update your password. Try requesting a new reset link.")},photoDelete:function(a){a.preventDefault();var b=$(a.target),a=b.parent().attr("action")+".json";OP.Util.makeRequest(a,b.parent().serializeArray(),function(a){204===a.code?(b.html("This photo has been deleted"),opTheme.message.confirm("This photo has been deleted.")):opTheme.message.error("Could not delete the photo.")});
return!1},photoEdit:function(a){a.preventDefault();a="/photo/"+$(a.target).attr("data-id")+"/edit.json";OP.Util.makeRequest(a,{},function(a){if(200===a.code){var c=$("#modal"),a=e.modal("Edit this photo",a.result.markup,null);c.html(a).modal();$(".typeahead",c).chosen()}else opTheme.message.error("Could not load the form to edit this photo.")},"json","get");return!1},photoView:function(a){a.preventDefault();a=$(a.target).parent();$(".photo-view");var b=a.attr("href"),a=b;if($("body").hasClass("photo-details"))location.href=
b;else{var c=$("#modal-photo-detail");$("#modal-photo-detail .photo-view");a=-1===a.indexOf("?")?a+"?modal=true":a+"&modal=true";c.load(a+" .photo-view",function(a){$("img.next-photo");$("img.previous-photo");f.insert(b,f.parse(a));l.fetchAndCacheNextPrevious()})}return!1},photoViewModal:function(a){a.preventDefault();a=$(a.target).parent();$(".photo-view");var b=a=a.attr("href"),c=$("#modal-photo-detail"),b=-1===b.indexOf("?")?b+"?modal=true":b+"&modal=true";location.hash=a;c.load(b+" .photo-view",
opTheme.callback.photoViewModalCb).modal().on("hidden",opTheme.callback.modalUnload);return!1},photoViewModalCb:function(a){l.fetchAndCacheNextPrevious();f.insert(location.hash,f.parse(a))},photoUpdate:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c=a.serialize();0===$('select[name^="groups"] option:checked',a).length&&(c+="&groups=");0===$('select[name^="albums"] option:checked',a).length&&(c+="&albums=");OP.Util.makeRequest(b,c,opTheme.callback.photoUpdateCb,"json",
"post")},photoUpdateBatch:function(a){a.preventDefault();var b=$(a.target),a=$("#batch-key").val(),c=$("form#batch-edit").find("*[name='value']"),d;b.html("Submitting...").attr("disabled","disabled");1==c.length?d=c.val():(b=$("form#batch-edit").find("*[name='value']:checked"),1==b.length?d=b.val():(d=[],b.each(function(a,b){d.push($(b).val())}),d=d.join(",")));b={crumb:j.get()};b[a]=d;b.ids=OP.Batch.collection.getIds().join(",");"delete"!==a?OP.Util.makeRequest("/photos/update.json",b,opTheme.callback.photoUpdateBatchCb,
"json","post"):OP.Util.makeRequest("/photos/delete.json",b,opTheme.callback.photoUpdateBatchCb,"json","post")},photoUpdateBatchCb:function(a){200==a.code?opTheme.message.append(e.message("Your photos were successfully updated.","confirm")):204==a.code?(OP.Batch.clear(),opTheme.message.append(e.message("Your photos were successfully deleted.","confirm"))):opTheme.message.append(e.message("There was a problem updating your photos.","error"));$("#modal").modal("hide")},photoUpdateCb:function(a){200===
a.code?opTheme.message.confirm("Your photo was successfully updated."):opTheme.message.error("We could not update your photo.");$("#modal").modal("hide")},photoUpload:function(a){a.preventDefault();a=$(a.target);a.attr("action");a.serialize()},photosViewMore:function(a){a.preventDefault();opTheme.init.pages.photos.load()},pinClick:function(a){var a=$(a.target),b=a.attr("data-id");a.hasClass("pinned")?OP.Batch.remove(b):OP.Batch.add(b)},pinClearClick:function(a){a.preventDefault();OP.Batch.clear()},
pluginStatus:function(a){a.preventDefault();a=$(a.target).attr("href")+".json";OP.Util.makeRequest(a,{},function(a){200===a.code?window.location.reload():opTheme.message.error("Could not update the status of this plugin.")},"json","post");return!1},pluginUpdate:function(a){a.preventDefault();var a=$(a.target).parent(),b=a.attr("action")+".json";OP.Util.makeRequest(b,a.serializeArray(),function(a){200===a.code?opTheme.message.confirm("Your plugin was successfully updated."):opTheme.message.error("Could not update the status of this plugin.")},
"json","post");return!1},preloadPhoto:function(a){OP.Util.makeRequest("/photo/"+a.id+"/view.json",{returnSizes:a.sizes,generate:"true"},opTheme.callback.preloadPhotoCb,"json","get")},preloadPhotoCb:function(a){var b=a.result;if(200===a.code)for(i in b)b.hasOwnProperty(i)&&!0===/^path[0-9]/.test(i)&&l.fetchAndCache(b[i])},searchByTags:function(a){a.preventDefault();var a=$(a.target),b=$($("select[name=tags]",a)[0]).val().join(","),c=a.attr("action");0<b.length&&(0<c.search("/list")?location.href=c.replace("/list",
"")+"/tags-"+b+"/list":a.submit());return!1},settings:function(){$("ul#settingsbar").slideToggle("medium");$("li#nav-signin").toggleClass("active");return!1},tagsInitialized:function(){var a=OP.Tag.getTags(),b="";if(null!==a&&0<a.length){for(i in a)b+='<option value="'+a[i]+'">'+a[i]+"</option>";$(".typeahead-tags").html(b).chosen()}},uploadCompleteSuccess:function(){$("form.upload").fadeOut("fast",function(){$(".upload-progress").fadeOut("fast",function(){$(".upload-complete").fadeIn("fast")});$(".upload-share").fadeIn("fast")})},
...
return!1}},init:{load:function(a){var b="state"in window.history,c=location.href;j.set(a);OP.Tag.init();m=location.pathname;History.Adapter.bind(window,"statechange",function(){var a=History.getState(),g=!b&&location.href==c;b=!0;g||f.render(a.data)});$(".dropdown-toggle").dropdown();"/"===location.pathname?opTheme.init.pages.front():"/manage"===location.pathname?opTheme.init.pages.manage():0===location.pathname.search(/^\/photos(.*)\/list/)?opTheme.init.pages.photos.init():0===location.pathname.search(/^\/p\/[a-z0-9]+/)||
0===location.pathname.search(/^\/photo\/[a-z0-9]+\/?(.*)\/view/)?opTheme.init.pages.photo.init():"/photos/upload"===location.pathname?opTheme.init.pages.upload():"/manage/albums"===location.pathname&&opTheme.init.pages.manageAlbums()},attach:function(){OP.Util.on("click:action-delete",opTheme.callback.actionDelete);OP.Util.on("click:action-jump",opTheme.callback.commentJump);OP.Util.on("click:action-post",opTheme.callback.actionPost);OP.Util.on("click:album-delete",opTheme.callback.albumDelete);OP.Util.on("click:album-form",
opTheme.callback.albumForm);OP.Util.on("click:album-show-all",opTheme.callback.albumShowAll);OP.Util.on("click:batch-modal",opTheme.callback.batchModal);OP.Util.on("click:credential-delete",opTheme.callback.credentailDelete);OP.Util.on("click:group-delete",opTheme.callback.groupDelete);OP.Util.on("click:group-email-add",opTheme.callback.groupEmailAdd);OP.Util.on("click:group-email-remove",opTheme.callback.groupEmailRemove);OP.Util.on("click:group-form",opTheme.callback.groupForm);OP.Util.on("click:login",
opTheme.callback.login);OP.Util.on("click:login-modal",opTheme.callback.loginModal);OP.Util.on("click:manage-password-request",opTheme.callback.passwordRequest);OP.Util.on("click:manage-password-reset",opTheme.callback.passwordReset);OP.Util.on("click:modal-close",opTheme.callback.modalClose);OP.Util.on("click:nav-item",opTheme.callback.searchBarToggle);OP.Util.on("click:photo-delete",opTheme.callback.photoDelete);OP.Util.on("click:photo-edit",opTheme.callback.photoEdit);OP.Util.on("click:photo-update-batch",
opTheme.callback.photoUpdateBatch);OP.Util.on("click:photo-view",opTheme.callback.photoView);OP.Util.on("click:photo-view-modal",opTheme.callback.photoViewModal);OP.Util.on("click:photos-load-more",opTheme.callback.photosViewMore);OP.Util.on("click:plugin-status",opTheme.callback.pluginStatus);OP.Util.on("click:plugin-update",opTheme.callback.pluginUpdate);OP.Util.on("click:pin",opTheme.callback.pinClick);OP.Util.on("click:pin-clear",opTheme.callback.pinClearClick);OP.Util.on("click:settings",opTheme.callback.settings);
OP.Util.on("click:webhook-delete",opTheme.callback.webhookDelete);OP.Util.on("submit:album-post",opTheme.callback.albumPost);OP.Util.on("submit:features-post",opTheme.callback.featuresPost);OP.Util.on("submit:group-post",opTheme.callback.groupPost);OP.Util.on("submit:login-openphoto",opTheme.callback.loginOpenPhoto);OP.Util.on("submit:photo-update",opTheme.callback.photoUpdate);OP.Util.on("submit:search",opTheme.callback.searchByTags);OP.Util.on("keydown:browse-next",opTheme.callback.keyBrowseNext);
OP.Util.on("keydown:browse-previous",opTheme.callback.keyBrowsePrevious);OP.Util.on("change:batch-field",opTheme.callback.batchField);OP.Util.on("callback:batch-add",opTheme.callback.batchAdd);OP.Util.on("callback:batch-remove",opTheme.callback.batchRemove);OP.Util.on("callback:batch-clear",opTheme.callback.batchClear);OP.Util.on("callback:tags-initialized",opTheme.callback.tagsInitialized);OP.Util.on("upload:complete-success",opTheme.callback.uploadCompleteSuccess);OP.Util.on("upload:complete-failure",
opTheme.callback.uploadCompleteFailure);OP.Util.on("tags:autocomplete",opTheme.callback.tagsAutocomplete);OP.Util.on("preload:photo",opTheme.callback.preloadPhoto)},pages:{front:function(){$(".carousel.front").swipe({swipeLeft:function(){("undefined"===typeof newBG||"undefined"===newBG)&&$(".carousel").carousel("next")},swipeRight:function(){("undefined"===typeof newBG||"undefined"===newBG)&&$(".carousel").carousel("prev")},threshold:100});$(".carousel.front").carousel({interval:7E3});$(".carousel.front").on("slid",
function(){var a=$(".carousel.front .active"),a=parseInt(a.attr("data-index"))+1;$(".carouselthumbs .active").removeClass("active");$(".carouselthumbs li:nth-child("+a+") a").addClass("active")});$(".carousel.feed").carousel();$(".carousel.feed").carousel("pause")},manage:function(){var a=OP.Batch.collection.getAll(),b=OP.Batch.collection.getLength(),c=$(".pin"),d,g;0<b&&opTheme.ui.batchMessage();c.each(function(b,c){c=$(c);d=c.attr("class");g=d.match(/ photo-([a-z0-9]+)/);2==g.length&&void 0!==a[g[1]]&&
c.addClass("revealed pinned")})},manageAlbums:function(){$("select.typeahead").chosen()},manageGroups:function(){$("select.typeahead").chosen()},photo:{init:function(){l.fetchAndCacheNextPrevious()}},photos:{initData:"undefined"===typeof initData?void 0:initData,albumContainerHeight:0,page:null,pageCount:0,end:!1,running:!1,init:function(){var a=opTheme.init.pages.photos;$(window).scroll(a.scrollCb);a.load()},scrollCb:function(){var a=opTheme.init.pages.photos;$(window).scrollTop()>$(document).height()-
$(window).height()-200&&a.load()},load:function(){var a=opTheme.init.pages.photos;loc=location;albumContainer=$(".album-list");if("undefined"===typeof a.initData){if(a.end||a.running)return;a.running=!0;if(null===a.page){var b=loc.href.match("page=([0-9]+)");null!==b?a.page=b[1]:(b=loc.pathname.match(/\/page-([0-9]+)/),null!==b&&(a.page=b[1]));null===a.page&&(a.page=1)}b=location.pathname+".json";params={};qs=window.location.search.replace("?","");if(0<qs.length){var c=qs.split("&"),d;for(i in c)c.hasOwnProperty(i)&&
(d=c[i].split("="),2===d.length&&(params[d[0]]=d[1]))}params.returnSizes="960x180";params.page=a.page;$.getJSON(b,params,a.loadCb)}else delete a.initData,a.page=1,a.loadCb({code:200,result:initData});1===albumContainer.length&&(b=$(albumContainer).outerHeight(),$(albumContainer).css({overflow:"auto",display:"table"}),c=$(albumContainer).outerHeight(),$(albumContainer).css({overflow:"hidden",display:"block"}),a.albumContainerHeight=[b,c],c>=b+100&&$(".show-all",albumContainer).show())},loadCb:function(a){var a=
a.result,b=opTheme.init.pages.photos,c=$(".infobar"),d=$(".startdate",c),c=$(".enddate",c);parseInt(d.attr("data-time"));var g=parseInt(c.attr("data-time")),h=opTheme.ui,f;if(a[0].totalPages>=b.page){var e;for(f=0;f<a.length;f++)e=parseInt(a[f].dateTaken),e>g?(h.fadeAndSet(c,phpjs.date("l F jS, Y",e)),c.attr("data-time",e),g=e):parseInt(a[f].dateTaken)<parseInt(d.attr("data-time"))&&(h.fadeAndSet(d,phpjs.date("l F jS, Y",e)),d.attr("data-time",e));Gallery.showImages($(".photo-grid-justify"),a);b.page++;
b.pageCount++;b.running=!1}else $(".load-more").hide(),b.end=!0}},upload:function(){$("form.upload");"object"===typeof OPU&&OPU.init();$("select.typeahead").chosen()}}},message:{append:function(a){var b=$(".message:first").clone(!1),c=$(".message:last");b.addClass("alert alert-info").html(a);c.after(b).slideDown()},confirm:function(a){opTheme.message.show(a,"confirm")},error:function(a){opTheme.message.show(a,"error")},show:function(a,b){opTheme.message.append(e.message(a,b))}},ui:{batchFormFields:{albums:function(){var a,
b="";$.ajax({url:"/albums/list.json",async:!1,success:function(c){if(200===c.code){b+='<label>Select albums</label><select name="value" multiple>';for(i in c.result)c.result.hasOwnProperty(i)&&(a=c.result[i],b+='<option value="'+a.id+'">'+a.name+"</option>");b+="</select>"}}});return b},empty:function(){return""},groups:function(){var a,b="";$.ajax({url:"/groups/list.json",async:!1,success:function(c){if(200===c.code){b+='<label>Select groups</label><select name="value" multiple>';for(i in c.result)c.result.hasOwnProperty(i)&&
(a=c.result[i],b+='<option value="'+a.id+'">'+a.name+"</option>");b+="</select>"}}});return b},permission:function(){return'  <label class="radio">    <input type="radio" name="value" value="1" checked="checked">    Public  </label>  <label class="radio">    <input type="radio" name="value" value="0">     Private  </label>'},tags:function(){return'  <label>Tags</label>  <input type="text" name="value" class="tags-autocomplete" placeholder="A comma separated list of tags" value="">'}},batchMessage:function(){var a=
OP.Batch.collection.getLength();0<$("#batch-message").length&&0<a?$("#batch-count").html(a):0==a?$("#batch-message").parent().parent().slideUp("fast",function(){$(this).html("").removeClass()}):opTheme.message.append(e.message('  <a id="batch-message"></a>You have <span id="batch-count">'+a+'</span> photos pinned.  <div><a class="btn small info batch-modal-click" data-controls-modal="modal" data-backdrop="static">Batch edit</a>&nbsp;<a href="#" class="btn small pin-clear-click">Or clear pins</a></div>'))},
fadeAndSet:function(a,b){var c=$(a);c.fadeOut("fast",function(){c.html(b).fadeIn("fast")})}},user:{base:{loginProcessed:function(a){200!=a.code?k("processing of login failed"):(k("login processing succeeded"),window.location.reload())}},browserid:{loginFailure:function(){k("login failed")},loginSuccess:function(a){OP.Util.makeRequest("/user/browserid/login.json",{assertion:a},opTheme.user.base.loginProcessed)}}}}}();
/* 09-phpjs.min.js */
