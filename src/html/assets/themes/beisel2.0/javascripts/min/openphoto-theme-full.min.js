...
f.modal(a.result.name,'<div class="clearfix">  <label>Consumer Key</label>  <p>'+a.result.id+"</p>  <label>Consumer Secret</label>  <p>"+a.result.clientSecret+"</p>  <label>Access Token</label>  <p>"+a.result.userToken+"</p>  <label>Access Token Secret</label>  <p>"+a.result.userSecret+"</p></div>",'<a href="#" class="btn" data-dismiss="modal">OK</a>');c.html(a).modal()}else opTheme.message.error("Could not load Application crendentials.")},"json","get");return!1},credentialDelete:function(a){a.preventDefault();
var b=$(a.target),a=b.attr("href")+".json";OP.Util.makeRequest(a,{},function(a){204===a.code?(b.parent().parent().slideUp("medium",function(){this.remove()}),opTheme.message.confirm("Application successfully deleted.")):opTheme.message.error("Could not delete Application.")});return!1},featuresPost:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c={},c={crumb:h.get()};c.allowDuplicate=$('input[name="allowDuplicate"]:checked',a).length;c.downloadOriginal=$('input[name="downloadOriginal"]:checked',
a).length;c.hideFromSearchEngines=$('input[name="hideFromSearchEngines"]:checked',a).length;OP.Util.makeRequest(b,c,opTheme.callback.featuresPostCb)},featuresPostCb:function(a){200===a.code?opTheme.message.confirm("Your features were successfully saved."):opTheme.message.error("We could not save your features.")},groupDelete:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+".json",c=b.parent(),b={crumb:h.get()};OP.Util.makeRequest(a,b,function(a){204===a.code?c.slideUp("medium",function(){this.remove()}):
opTheme.message.error("We could not delete this group.")})},groupEmailAdd:function(a){a.preventDefault();var a=$(a.target).prev(),b=$("ul.group-emails-add-list",a.parent()),c=a.val();""!==c&&($('<li><span class="group-email-queue">'+c+'</span> <a href="#" class="group-email-remove-click"><i class="group-email-remove-click icon-minus-sign"></i></a></li>').prependTo(b),a.val(""))},groupEmailRemove:function(a){a.preventDefault();$(a.target).parent().parent().remove()},groupForm:function(a){a.preventDefault();
a=$(a.target).attr("href")+".json";OP.Util.makeRequest(a,{modal:"true",dynamic:"true"},function(a){if(200===a.code){var c=$("#modal"),a=f.modal("Create a group",a.result.markup,null);c.html(a).modal()}else opTheme.message.error("Could not load the form to create an album.")},"json","get");return!1},groupPost:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c=-1<b.search("create"),d=$('input[name="dynamic"]',a).val(),e,f={name:$('input[name="name"]',a).val()};f.crumb=h.get();
$(".group-email-add-click",a).trigger("click");e=[];$("span.group-email-queue",a).each(function(a,b){e.push($(b).html())});f.members=e.join(",");OP.Util.makeRequest(b,f,function(a){var b;200===a.code||201===a.code?d?opTheme.callback.groupPostDynamicCb(b,a.result):c?location.href="/manage/groups?m=group-created":opTheme.message.confirm("Group updated successfully."):opTheme.message.error("Could not update group.")})},groupPostDynamicCb:function(a,b){var c=$('select[name="groups"]',a);$(".modal").modal("hide");
...
f.modal("Edit this photo",a.result.markup,null);c.html(a).modal();$(".typeahead",c).chosen()}else opTheme.message.error("Could not load the form to edit this photo.")},"json","get");return!1},photoView:function(a){a.preventDefault();a=$(a.target).parent();$(".photo-view");var b=a.attr("href"),a=b;if($("body").hasClass("photo-details"))location.href=b;else{var c=$("#modal-photo-detail");$("#modal-photo-detail .photo-view");a=-1===a.indexOf("?")?a+"?modal=true":a+"&modal=true";c.load(a+" .photo-view",
function(a){$("img.next-photo");$("img.previous-photo");g.insert(b,g.parse(a));k.fetchAndCacheNextPrevious()})}return!1},photoViewModal:function(a){if(!(900>k.getDeviceWidth())){a.preventDefault();a=$(a.target).parent();$(".photo-view");var b=a=a.attr("href"),c=$("#modal-photo-detail"),b=-1===b.indexOf("?")?b+"?modal=true":b+"&modal=true";location.hash=a;c.load(b+" .photo-view",opTheme.callback.photoViewModalCb).modal().on("hidden",opTheme.callback.modalUnload);return!1}},photoViewModalCb:function(a){k.fetchAndCacheNextPrevious();
g.insert(location.hash,g.parse(a))},photoUpdate:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c=a.serialize();0===$('select[name^="groups"] option:checked',a).length&&(c+="&groups=");0===$('select[name^="albums"] option:checked',a).length&&(c+="&albums=");OP.Util.makeRequest(b,c,opTheme.callback.photoUpdateCb,"json","post")},photoUpdateBatch:function(a){a.preventDefault();var b=$(a.target),a=$("#batch-key").val(),c=$("form#batch-edit").find("*[name='value']"),d;b.html("Submitting...").attr("disabled",
"disabled");1==c.length?d=c.val():(b=$("form#batch-edit").find("*[name='value']:checked"),1==b.length?d=b.val():(d=[],b.each(function(a,b){d.push($(b).val())}),d=d.join(",")));b={crumb:h.get()};b[a]=d;b.ids=OP.Batch.collection.getIds().join(",");"delete"!==a?OP.Util.makeRequest("/photos/update.json",b,opTheme.callback.photoUpdateBatchCb,"json","post"):OP.Util.makeRequest("/photos/delete.json",b,opTheme.callback.photoUpdateBatchCb,"json","post")},photoUpdateBatchCb:function(a){200==a.code?opTheme.message.append(f.message("Your photos were successfully updated.",
"confirm")):204==a.code?(OP.Batch.clear(),opTheme.message.append(f.message("Your photos were successfully deleted.","confirm"))):opTheme.message.append(f.message("There was a problem updating your photos.","error"));$("#modal").modal("hide")},photoUpdateCb:function(a){200===a.code?opTheme.message.confirm("Your photo was successfully updated."):opTheme.message.error("We could not update your photo.");$("#modal").modal("hide")},photoUpload:function(a){a.preventDefault();a=$(a.target);a.attr("action");
a.serialize()},photosViewMore:function(a){a.preventDefault();opTheme.init.pages.photos.load()},pinClick:function(a){var a=$(a.target),b=a.attr("data-id");a.hasClass("pinned")?OP.Batch.remove(b):OP.Batch.add(b)},pinClearClick:function(a){a.preventDefault();OP.Batch.clear()},pinSelectAll:function(a){a.preventDefault();$(".pin").each(function(){id=$(this).attr("data-id");container=$(this).parent();container.hasClass("pinned")||OP.Batch.add(id)})},pluginView:function(a){a.preventDefault();var a=$(a.target).attr("href")+
".json",b={crumb:h.get()},c=a.match(/\/plugin\/(.*)\/view.json$/);OP.Util.makeRequest("/plugin/"+c[1]+"/view.json",b,function(a){200===a.code?$("#modal").html(f.modal("Update "+c[1],a.result.markup,null)).modal():opTheme.message.error("Unable to load this plugin for editing.")},"json","get")},pluginStatusToggle:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+".json";OP.Util.makeRequest(a,{},function(a){var d=$(b).parent(),e=d.parent();200===a.code?($("div",e).removeClass("hide"),
d.addClass("hide")):opTheme.message.error("Could not update the status of this plugin.")},"json","post");return!1},pluginUpdate:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json";OP.Util.makeRequest(b,a.serializeArray(),function(a){200===a.code?opTheme.message.confirm("Your plugin was successfully updated."):opTheme.message.error("Could not update the status of this plugin.");$("#modal").modal("hide")},"json","post");return!1},preloadPhoto:function(a){OP.Util.makeRequest("/photo/"+
a.id+"/view.json",{returnSizes:a.sizes,generate:"true"},opTheme.callback.preloadPhotoCb,"json","get")},preloadPhotoCb:function(a){var b=a.result;if(200===a.code)for(i in b)b.hasOwnProperty(i)&&!0===/^path[0-9]/.test(i)&&k.fetchAndCache(b[i])},popup:function(a){a.preventDefault();var b=$(a.target),a=b.attr("data-width"),c=b.attr("data-height"),b=b.attr("href");window.open(b,"openphoto","toolbar=0,menubar=0,location=1,directories=0,channelmode=0,width="+a+",height="+c)},searchByTags:function(a){a.preventDefault();
var a=$(a.target),b=$($("select[name=tags]",a)[0]).val().join(","),c=a.attr("action");0<b.length&&(0<c.search("/list")?location.href=c.replace("/list","")+"/tags-"+b+"/list":a.submit());return!1},settings:function(){$("ul#settingsbar").slideToggle("medium");$("li#nav-signin").toggleClass("active");return!1},share:{facebook:function(a){a.preventDefault();var a=$(a.target),b={method:"feed",display:"popup"};b.link=a.attr("data-link");b.picture=a.attr("data-picture");b.name=a.attr("data-name");b.description=
a.attr("data-description");FB.ui(b,function(){window.close()})}},tagsInitialized:function(){var a=OP.Tag.getTags(),b="";if(null!==a&&0<a.length)for(i in a)b+='<option value="'+a[i]+'">'+a[i]+"</option>";$(".typeahead-tags").html(b).chosen()},uploadCompleteSuccess:function(a){$("form.upload").fadeOut("fast",function(){OP.Util.makeRequest("/photos/upload/confirm.json",a,opTheme.callback.uploadConfirm,"json","post")})},uploadCompleteFailure:function(){$("form.upload").fadeOut("fast",function(){$(".upload-progress").fadeOut("fast",
function(){$(".upload-warning .failed").html(failed);$(".upload-warning .total").html(total);$(".upload-warning").fadeIn("fast")})})},uploadConfirm:function(a){$("body.upload .upload-container").fadeOut("fast",function(){$(".upload-confirm").fadeIn("fast")});$("body.upload .upload-confirm").html(a.result).show("fast")},webhookDelete:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+".json";OP.Util.makeRequest(a,{},function(a){204===a.code?(b.parent().remove(),opTheme.message.confirm("Credential successfully deleted.")):
opTheme.message.error("Could not delete credential.")});return!1}},init:{load:function(a){var b="state"in window.history,c=location.href;h.set(a);OP.Tag.init();l=location.pathname;History.Adapter.bind(window,"statechange",function(){var a=History.getState(),e=!b&&location.href==c;b=!0;e||g.render(a.data)});$(".dropdown-toggle").dropdown();$(".modal").on("shown",opTheme.callback.modalShown);"/"===location.pathname?opTheme.init.pages.front():"/manage/photos"===location.pathname?opTheme.init.pages.manage.photos():
...
location.href=a.pageLocation.pathname+"?"+decodeURIComponent($.param(params)):$.getJSON(b,params,a.loadCb)}}else delete a.initData,a.page=1,a.loadCb({code:200,result:initData})},loadCb:function(a){var a=a.result,b=opTheme.init.pages.photos,c=$(".infobar"),d=$(".startdate",c),c=$(".enddate",c);parseInt(d.attr("data-time"));var e=parseInt(c.attr("data-time")),f=opTheme.ui,g;if(a[0].totalPages>=b.page){var h;for(g=0;g<a.length;g++)h=parseInt(a[g].dateTaken),h>e?(f.fadeAndSet(c,phpjs.date("l F jS, Y",
h)),c.attr("data-time",h),e=h):parseInt(a[g].dateTaken)<parseInt(d.attr("data-time"))&&(f.fadeAndSet(d,phpjs.date("l F jS, Y",h)),d.attr("data-time",h));Gallery.showImages($(".photo-grid-justify"),a);b.page++;b.pageCount++;b.running=!1}else $(".load-more").hide(),b.end=!0}},upload:function(){$("form.upload");"object"===typeof OPU&&OPU.init();$("select.typeahead").chosen()}}},message:{append:function(a){var b=$(".message:first").clone(!1),c=$(".message:last");b.addClass("alert alert-info").html(a);
c.after(b).slideDown()},confirm:function(a){opTheme.message.show(a,"confirm")},error:function(a){opTheme.message.show(a,"error")},show:function(a,b){opTheme.message.append(f.message(a,b))}},ui:{batchFormFields:{albums:function(){var a,b="";$.ajax({url:"/albums/list.json",data:{pageSize:"0"},async:!1,success:function(c){if(200===c.code){b+='<label>Select albums</label><select name="value" multiple>';for(i in c.result)c.result.hasOwnProperty(i)&&(a=c.result[i],b+='<option value="'+a.id+'">'+a.name+
...
tags:function(){return'  <label>Tags</label>  <input type="text" name="value" class="tags-autocomplete" placeholder="A comma separated list of tags" value="">'}},batchMessage:function(){var a=OP.Batch.collection.getLength();0<$("#batch-message").length&&0<a?$("#batch-count").html(a):0==a?$("#batch-message").parent().parent().slideUp("fast",function(){$(this).html("").removeClass()}):opTheme.message.append('<div class="batch-message-container">'+f.message('  <a id="batch-message"></a>You have <span id="batch-count">'+
a+'</span> photos pinned.  <div><a class="btn small info batch-modal-click" data-controls-modal="modal" data-backdrop="static">Batch edit</a>&nbsp;<a href="#" class="btn small pin-clear-click">Or clear pins</a></div>')+"</div>")},fadeAndSet:function(a,b){var c=$(a);c.fadeOut("fast",function(){c.html(b).fadeIn("fast")})}},user:{base:{loginProcessed:function(a){200!=a.code?j("processing of login failed"):(j("login processing succeeded"),window.location.reload())}},browserid:{loginFailure:function(){j("login failed")},
loginSuccess:function(a){OP.Util.makeRequest("/user/browserid/login.json",{assertion:a},opTheme.user.base.loginProcessed)}}}}}();
/* 09-phpjs.min.js */
