...
c=$(".pin"),d,e;0<b&&opTheme.ui.batchMessage();c.each(function(b,c){c=$(c);d=c.attr("class");e=d.match(/ photo-([a-z0-9]+)/);2==e.length&&void 0!==a[e[1]]&&c.addClass("revealed pinned")})},manageAlbums:function(){$("select.typeahead").chosen()},manageGroups:function(){$("select.typeahead").chosen()},photo:{init:function(){k.fetchAndCacheNextPrevious()}},photos:{initData:"undefined"===typeof initData?void 0:initData,albumContainerHeight:0,page:null,pageCount:0,maxMobilePageCount:5,end:!1,running:!1,
init:function(){var a=opTheme.init.pages.photos;$(window).scroll(a.scrollCb);a.load()},scrollCb:function(){var a=opTheme.init.pages.photos;900>k.getDeviceWidth()||$(window).scrollTop()>$(document).height()-$(window).height()-200&&a.load()},load:function(){var a=opTheme.init.pages.photos;loc=location;albumContainer=$(".album-list");if("undefined"===typeof a.initData){if(a.end||a.running)return;a.running=!0;if(null===a.page){var b=loc.href.match("page=([0-9]+)");null!==b?a.page=b[1]:(b=loc.pathname.match(/\/page-([0-9]+)/),
null!==b&&(a.page=b[1]));null===a.page&&(a.page=1)}b=location.pathname+".json";params={};qs=window.location.search.replace("?","");if(0<qs.length){var c=qs.split("&"),d;for(i in c)c.hasOwnProperty(i)&&(d=c[i].split("="),2===d.length&&(params[d[0]]=d[1]))}params.returnSizes="960x180";params.page=a.page;a.pageCount>a.maxMobilePageCount&&900>k.getDeviceWidth()?location.href=location.pathname+"?"+decodeURIComponent($.param(params)):$.getJSON(b,params,a.loadCb)}else delete a.initData,a.page=1,a.loadCb({code:200,
result:initData});1===albumContainer.length&&(b=$(albumContainer).outerHeight(),$(albumContainer).css({overflow:"auto",display:"table"}),c=$(albumContainer).outerHeight(),$(albumContainer).css({overflow:"hidden",display:"block"}),a.albumContainerHeight=[b,c],c>=b+100&&$(".show-all",albumContainer).show())},loadCb:function(a){var a=a.result,b=opTheme.init.pages.photos,c=$(".infobar"),d=$(".startdate",c),c=$(".enddate",c);parseInt(d.attr("data-time"));var e=parseInt(c.attr("data-time")),f=opTheme.ui,
h;if(a[0].totalPages>=b.page){var g;for(h=0;h<a.length;h++)g=parseInt(a[h].dateTaken),g>e?(f.fadeAndSet(c,phpjs.date("l F jS, Y",g)),c.attr("data-time",g),e=g):parseInt(a[h].dateTaken)<parseInt(d.attr("data-time"))&&(f.fadeAndSet(d,phpjs.date("l F jS, Y",g)),d.attr("data-time",g));Gallery.showImages($(".photo-grid-justify"),a);b.page++;b.pageCount++;b.running=!1}else $(".load-more").hide(),b.end=!0}},upload:function(){$("form.upload");"object"===typeof OPU&&OPU.init();$("select.typeahead").chosen()}}},
message:{append:function(a){var b=$(".message:first").clone(!1),c=$(".message:last");b.addClass("alert alert-info").html(a);c.after(b).slideDown()},confirm:function(a){opTheme.message.show(a,"confirm")},error:function(a){opTheme.message.show(a,"error")},show:function(a,b){opTheme.message.append(h.message(a,b))}},ui:{batchFormFields:{albums:function(){var a,b="";$.ajax({url:"/albums/list.json",async:!1,success:function(c){if(200===c.code){b+='<label>Select albums</label><select name="value" multiple>';
for(i in c.result)c.result.hasOwnProperty(i)&&(a=c.result[i],b+='<option value="'+a.id+'">'+a.name+"</option>");b+="</select>"}}});return b},empty:function(){return""},groups:function(){var a,b="";$.ajax({url:"/groups/list.json",async:!1,success:function(c){if(200===c.code){b+='<label>Select groups</label><select name="value" multiple>';for(i in c.result)c.result.hasOwnProperty(i)&&(a=c.result[i],b+='<option value="'+a.id+'">'+a.name+"</option>");b+="</select>"}}});return b},permission:function(){return'  <label class="radio">    <input type="radio" name="value" value="1" checked="checked">    Public  </label>  <label class="radio">    <input type="radio" name="value" value="0">     Private  </label>'},
tags:function(){return'  <label>Tags</label>  <input type="text" name="value" class="tags-autocomplete" placeholder="A comma separated list of tags" value="">'}},batchMessage:function(){var a=OP.Batch.collection.getLength();0<$("#batch-message").length&&0<a?$("#batch-count").html(a):0==a?$("#batch-message").parent().parent().slideUp("fast",function(){$(this).html("").removeClass()}):opTheme.message.append(h.message('  <a id="batch-message"></a>You have <span id="batch-count">'+a+'</span> photos pinned.  <div><a class="btn small info batch-modal-click" data-controls-modal="modal" data-backdrop="static">Batch edit</a>&nbsp;<a href="#" class="btn small pin-clear-click">Or clear pins</a></div>'))},
