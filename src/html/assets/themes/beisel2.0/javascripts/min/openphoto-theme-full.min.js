...
"json","get");return!1},albumPost:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c=-1<b.search("create"),d=parseInt($('input[name="dynamic"]',a).val()),e={};e.name=$('input[name="name"]',a).val();e.crumb=g.get();0==e.name.length?opTheme.message.error("Please enter a name for your album."):OP.Util.makeRequest(b,e,function(a){var b;200===a.code||201===a.code?d?opTheme.callback.albumPostDynamicCb(b,a.result):c?location.href="/manage/albums?m=album-created&rand="+parseInt(1E5*
Math.random())+"#album-"+a.result.id:opTheme.message.confirm("Album updated successfully."):opTheme.message.error("Could not update album.")})},albumPostDynamicCb:function(a,b){var c=$('select[name="albums"]',a);$(".modal").modal("hide");$('<option value="'+b.id+'" selected="selected">'+b.name+"</option>").prependTo(c);c.trigger("liszt:updated")},albumShowAll:function(a){a.preventDefault();var a=$(".album-list"),b=a.outerHeight(),c=opTheme.init.pages.photos.albumContainerHeight[1],d=opTheme.init.pages.photos.albumContainerHeight[0],
e={height:0};e.height=b==c?d+"px":c+"px";a.animate(e)},batchAdd:function(a){$(".pin.photo-"+a.id).addClass("revealed pinned");opTheme.ui.batchMessage();j("Adding photo "+a.id)},batchClear:function(){$(".pin.pinned").removeClass("revealed pinned");opTheme.ui.batchMessage()},batchField:function(a){var a=$(a.target).val(),b=$("form#batch-edit .form-fields");switch(a){case "albumsAdd":b.html(opTheme.ui.batchFormFields.albums());$("select",b).chosen();break;case "delete":b.html(opTheme.ui.batchFormFields.empty());
break;case "groups":b.html(opTheme.ui.batchFormFields.groups());$("select",b).chosen();break;case "permission":b.html(opTheme.ui.batchFormFields.permission());break;case "tagsAdd":case "tagsRemove":b.html(opTheme.ui.batchFormFields.tags())}},batchModal:function(){var a=$("#modal"),b=h.modal("Batch edit your pinned photos",'<form id="batch-edit">  <div class="clearfix">    <label>What would you like to do?</label>    <div class="input">      <select id="batch-key" class="batch-field-change" name="property">        <option value="tagsAdd">Add Tags</option>        <option value="tagsRemove">Remove Tags</option>        <option value="albumsAdd">Add Albums</option>        <option value="groups">Update Groups</option>        <option value="permission">Update Permissions</option>        <option value="delete">Delete</option>      </select>    </div>  </div>  <div class="form-fields">'+
...
null);c.html(a).modal()}else opTheme.message.error("Could not load the form to create an album.")},"json","get");return!1},groupPost:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c=-1<b.search("create"),d=$('input[name="dynamic"]',a).val(),e,f={name:$('input[name="name"]',a).val()};f.crumb=g.get();$(".group-email-add-click",a).trigger("click");e=[];$("span.group-email-queue",a).each(function(a,b){e.push($(b).html())});f.members=e.join(",");OP.Util.makeRequest(b,f,function(a){var b;
...
$(a.target).parent().parent().slideUp("fast",function(){$(this).remove()})},modalShown:function(a){a=$('input[type="text"]',a.target);0<a.length&&a[0].focus()},modalUnload:function(a){a.preventDefault();$("#modal-photo-detail").html("");f.replace(l)},passwordRequest:function(a){a.preventDefault();a=$(a.target).parent();a=$("#login-email",a).val();OP.Util.makeRequest("/user/password/request.json",{email:a},opTheme.callback.passwordRequestCb)},passwordRequestCb:function(a){$("#loginBox").modal("hide");
...
var b=a.attr("href"),a=b;if($("body").hasClass("photo-details"))location.href=b;else{var c=$("#modal-photo-detail");$("#modal-photo-detail .photo-view");a=-1===a.indexOf("?")?a+"?modal=true":a+"&modal=true";c.load(a+" .photo-view",function(a){$("img.next-photo");$("img.previous-photo");f.insert(b,f.parse(a));k.fetchAndCacheNextPrevious()})}return!1},photoViewModal:function(a){if(!(900>k.getDeviceWidth())){a.preventDefault();a=$(a.target).parent();$(".photo-view");var b=a=a.attr("href"),c=$("#modal-photo-detail"),
...
pinClearClick:function(a){a.preventDefault();OP.Batch.clear()},pluginStatus:function(a){a.preventDefault();a=$(a.target).attr("href")+".json";OP.Util.makeRequest(a,{},function(a){200===a.code?window.location.reload():opTheme.message.error("Could not update the status of this plugin.")},"json","post");return!1},pluginUpdate:function(a){a.preventDefault();var a=$(a.target).parent(),b=a.attr("action")+".json";OP.Util.makeRequest(b,a.serializeArray(),function(a){200===a.code?opTheme.message.confirm("Your plugin was successfully updated."):
opTheme.message.error("Could not update the status of this plugin.")},"json","post");return!1},preloadPhoto:function(a){OP.Util.makeRequest("/photo/"+a.id+"/view.json",{returnSizes:a.sizes,generate:"true"},opTheme.callback.preloadPhotoCb,"json","get")},preloadPhotoCb:function(a){var b=a.result;if(200===a.code)for(i in b)b.hasOwnProperty(i)&&!0===/^path[0-9]/.test(i)&&k.fetchAndCache(b[i])},searchByTags:function(a){a.preventDefault();var a=$(a.target),b=$($("select[name=tags]",a)[0]).val().join(","),
c=a.attr("action");0<b.length&&(0<c.search("/list")?location.href=c.replace("/list","")+"/tags-"+b+"/list":a.submit());return!1},settings:function(){$("ul#settingsbar").slideToggle("medium");$("li#nav-signin").toggleClass("active");return!1},tagsInitialized:function(){var a=OP.Tag.getTags(),b="";if(null!==a&&0<a.length){for(i in a)b+='<option value="'+a[i]+'">'+a[i]+"</option>";$(".typeahead-tags").html(b).chosen()}},uploadCompleteSuccess:function(){$("form.upload").fadeOut("fast",function(){$(".upload-progress").fadeOut("fast",
function(){$(".upload-complete").fadeIn("fast")});$(".upload-share").fadeIn("fast")})},uploadCompleteFailure:function(){$("form.upload").fadeOut("fast",function(){$(".upload-progress").fadeOut("fast",function(){$(".upload-warning .failed").html(failed);$(".upload-warning .total").html(total);$(".upload-warning").fadeIn("fast")})})},webhookDelete:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+".json";OP.Util.makeRequest(a,{},function(a){204===a.code?(b.parent().remove(),opTheme.message.confirm("Credential successfully deleted.")):
opTheme.message.error("Could not delete credential.")});return!1}},init:{load:function(a){var b="state"in window.history,c=location.href;g.set(a);OP.Tag.init();l=location.pathname;History.Adapter.bind(window,"statechange",function(){var a=History.getState(),e=!b&&location.href==c;b=!0;e||f.render(a.data)});$(".dropdown-toggle").dropdown();$(".modal").on("shown",opTheme.callback.modalShown);"/"===location.pathname?opTheme.init.pages.front():"/manage"===location.pathname?opTheme.init.pages.manage():
0===location.pathname.search(/^\/photos(.*)\/list/)?opTheme.init.pages.photos.init():0===location.pathname.search(/^\/p\/[a-z0-9]+/)||0===location.pathname.search(/^\/photo\/[a-z0-9]+\/?(.*)\/view/)?opTheme.init.pages.photo.init():"/photos/upload"===location.pathname?opTheme.init.pages.upload():"/manage/albums"===location.pathname&&opTheme.init.pages.manageAlbums()},attach:function(){OP.Util.on("click:action-delete",opTheme.callback.actionDelete);OP.Util.on("click:action-jump",opTheme.callback.commentJump);
OP.Util.on("click:action-post",opTheme.callback.actionPost);OP.Util.on("click:album-delete",opTheme.callback.albumDelete);OP.Util.on("click:album-form",opTheme.callback.albumForm);OP.Util.on("click:album-show-all",opTheme.callback.albumShowAll);OP.Util.on("click:batch-modal",opTheme.callback.batchModal);OP.Util.on("click:credential-delete",opTheme.callback.credentailDelete);OP.Util.on("click:group-delete",opTheme.callback.groupDelete);OP.Util.on("click:group-email-add",opTheme.callback.groupEmailAdd);
OP.Util.on("click:group-email-remove",opTheme.callback.groupEmailRemove);OP.Util.on("click:group-form",opTheme.callback.groupForm);OP.Util.on("click:login",opTheme.callback.login);OP.Util.on("click:login-modal",opTheme.callback.loginModal);OP.Util.on("click:manage-password-request",opTheme.callback.passwordRequest);OP.Util.on("click:manage-password-reset",opTheme.callback.passwordReset);OP.Util.on("click:modal-close",opTheme.callback.modalClose);OP.Util.on("click:nav-item",opTheme.callback.searchBarToggle);
OP.Util.on("click:photo-delete",opTheme.callback.photoDelete);OP.Util.on("click:photo-edit",opTheme.callback.photoEdit);OP.Util.on("click:photo-update-batch",opTheme.callback.photoUpdateBatch);OP.Util.on("click:photo-view",opTheme.callback.photoView);OP.Util.on("click:photo-view-modal",opTheme.callback.photoViewModal);OP.Util.on("click:photos-load-more",opTheme.callback.photosViewMore);OP.Util.on("click:plugin-status",opTheme.callback.pluginStatus);OP.Util.on("click:plugin-update",opTheme.callback.pluginUpdate);
OP.Util.on("click:pin",opTheme.callback.pinClick);OP.Util.on("click:pin-clear",opTheme.callback.pinClearClick);OP.Util.on("click:settings",opTheme.callback.settings);OP.Util.on("click:webhook-delete",opTheme.callback.webhookDelete);OP.Util.on("submit:album-post",opTheme.callback.albumPost);OP.Util.on("submit:features-post",opTheme.callback.featuresPost);OP.Util.on("submit:group-post",opTheme.callback.groupPost);OP.Util.on("submit:login-openphoto",opTheme.callback.loginOpenPhoto);OP.Util.on("submit:photo-update",
opTheme.callback.photoUpdate);OP.Util.on("submit:search",opTheme.callback.searchByTags);OP.Util.on("keydown:browse-next",opTheme.callback.keyBrowseNext);OP.Util.on("keydown:browse-previous",opTheme.callback.keyBrowsePrevious);OP.Util.on("change:batch-field",opTheme.callback.batchField);OP.Util.on("callback:batch-add",opTheme.callback.batchAdd);OP.Util.on("callback:batch-remove",opTheme.callback.batchRemove);OP.Util.on("callback:batch-clear",opTheme.callback.batchClear);OP.Util.on("callback:tags-initialized",
opTheme.callback.tagsInitialized);OP.Util.on("upload:complete-success",opTheme.callback.uploadCompleteSuccess);OP.Util.on("upload:complete-failure",opTheme.callback.uploadCompleteFailure);OP.Util.on("tags:autocomplete",opTheme.callback.tagsAutocomplete);OP.Util.on("preload:photo",opTheme.callback.preloadPhoto)},pages:{front:function(){$(".carousel.front").swipe({swipeLeft:function(){("undefined"===typeof newBG||"undefined"===newBG)&&$(".carousel").carousel("next")},swipeRight:function(){("undefined"===
typeof newBG||"undefined"===newBG)&&$(".carousel").carousel("prev")},threshold:100});$(".carousel.front").carousel({interval:7E3});$(".carousel.front").on("slid",function(){var a=$(".carousel.front .active"),a=parseInt(a.attr("data-index"))+1;$(".carouselthumbs .active").removeClass("active");$(".carouselthumbs li:nth-child("+a+") a").addClass("active")});$(".carousel.feed").carousel();$(".carousel.feed").carousel("pause")},manage:function(){var a=OP.Batch.collection.getAll(),b=OP.Batch.collection.getLength(),
c=$(".pin"),d,e;0<b&&opTheme.ui.batchMessage();c.each(function(b,c){c=$(c);d=c.attr("class");e=d.match(/ photo-([a-z0-9]+)/);2==e.length&&void 0!==a[e[1]]&&c.addClass("revealed pinned")})},manageAlbums:function(){$("select.typeahead").chosen()},manageGroups:function(){$("select.typeahead").chosen()},photo:{init:function(){k.fetchAndCacheNextPrevious()}},photos:{initData:"undefined"===typeof initData?void 0:initData,filterOpts:"undefined"===typeof filterOpts?void 0:filterOpts,albumContainerHeight:0,
page:null,pageCount:0,pageLocation:window.location,maxMobilePageCount:5,end:!1,running:!1,init:function(){var a=opTheme.init.pages.photos;$(window).scroll(a.scrollCb);a.load()},scrollCb:function(){var a=opTheme.init.pages.photos;900>k.getDeviceWidth()||$(window).scrollTop()>$(document).height()-$(window).height()-200&&a.load()},load:function(){var a=opTheme.init.pages.photos;loc=location;albumContainer=$(".album-list");if("undefined"===typeof a.initData){if(a.end||a.running)return;a.running=!0;if(null===
a.page){var b=loc.href.match("page=([0-9]+)");null!==b?a.page=b[1]:(b=loc.pathname.match(/\/page-([0-9]+)/),null!==b&&(a.page=b[1]));null===a.page&&(a.page=1)}b=a.pageLocation.pathname+".json";params={};qs=a.pageLocation.search.replace("?","");if(0<qs.length){var c=qs.split("&"),d;for(i in c)c.hasOwnProperty(i)&&(d=c[i].split("="),2===d.length&&(params[d[0]]=d[1]))}params.returnSizes="960x180";params.page=a.page;a.pageCount>a.maxMobilePageCount&&900>k.getDeviceWidth()?location.href=a.pageLocation.pathname+
"?"+decodeURIComponent($.param(params)):$.getJSON(b,params,a.loadCb)}else delete a.initData,a.page=1,a.loadCb({code:200,result:initData});1===albumContainer.length&&(b=$(albumContainer).outerHeight(),$(albumContainer).css({overflow:"auto",display:"table"}),c=$(albumContainer).outerHeight(),$(albumContainer).css({overflow:"hidden",display:"block"}),a.albumContainerHeight=[b,c],c>=b+100&&$(".show-all",albumContainer).show())},loadCb:function(a){var a=a.result,b=opTheme.init.pages.photos,c=$(".infobar"),
d=$(".startdate",c),c=$(".enddate",c);parseInt(d.attr("data-time"));var e=parseInt(c.attr("data-time")),f=opTheme.ui,h;if(a[0].totalPages>=b.page){var g;for(h=0;h<a.length;h++)g=parseInt(a[h].dateTaken),g>e?(f.fadeAndSet(c,phpjs.date("l F jS, Y",g)),c.attr("data-time",g),e=g):parseInt(a[h].dateTaken)<parseInt(d.attr("data-time"))&&(f.fadeAndSet(d,phpjs.date("l F jS, Y",g)),d.attr("data-time",g));Gallery.showImages($(".photo-grid-justify"),a);b.page++;b.pageCount++;b.running=!1}else $(".load-more").hide(),
b.end=!0}},upload:function(){$("form.upload");"object"===typeof OPU&&OPU.init();$("select.typeahead").chosen()}}},message:{append:function(a){var b=$(".message:first").clone(!1),c=$(".message:last");b.addClass("alert alert-info").html(a);c.after(b).slideDown()},confirm:function(a){opTheme.message.show(a,"confirm")},error:function(a){opTheme.message.show(a,"error")},show:function(a,b){opTheme.message.append(h.message(a,b))}},ui:{batchFormFields:{albums:function(){var a,b="";$.ajax({url:"/albums/list.json",
