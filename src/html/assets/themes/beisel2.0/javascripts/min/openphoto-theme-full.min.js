...
f.modal(a.result.name,'<div class="clearfix">  <label>Consumer Key</label>  <p>'+a.result.id+"</p>  <label>Consumer Secret</label>  <p>"+a.result.clientSecret+"</p>  <label>Access Token</label>  <p>"+a.result.userToken+"</p>  <label>Access Token Secret</label>  <p>"+a.result.userSecret+"</p></div>",'<a href="#" class="btn credential-view-dialogue-ok-click">OK</a>');c.html(a).modal()}else opTheme.message.error("Could not load Application crendentials.")},"json","get");return!1},credentialViewDialogueOk:function(a){a.preventDefault();
$(".modal").modal("hide")},credentialDelete:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+".json";OP.Util.makeRequest(a,{},function(a){204===a.code?(b.parent().parent().slideUp("medium",function(){this.remove()}),opTheme.message.confirm("Application successfully deleted.")):opTheme.message.error("Could not delete Application.")});return!1},featuresPost:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c={},c={crumb:h.get()};c.allowDuplicate=$('input[name="allowDuplicate"]:checked',
a).length;c.downloadOriginal=$('input[name="downloadOriginal"]:checked',a).length;c.hideFromSearchEngines=$('input[name="hideFromSearchEngines"]:checked',a).length;OP.Util.makeRequest(b,c,opTheme.callback.featuresPostCb)},featuresPostCb:function(a){200===a.code?opTheme.message.confirm("Your features were successfully saved."):opTheme.message.error("We could not save your features.")},groupDelete:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+".json",c=b.parent(),b={crumb:h.get()};
OP.Util.makeRequest(a,b,function(a){204===a.code?c.slideUp("medium",function(){this.remove()}):opTheme.message.error("We could not delete this group.")})},groupEmailAdd:function(a){a.preventDefault();var a=$(a.target).prev(),b=$("ul.group-emails-add-list",a.parent()),c=a.val();""!==c&&($('<li><span class="group-email-queue">'+c+'</span> <a href="#" class="group-email-remove-click"><i class="group-email-remove-click icon-minus-sign"></i></a></li>').prependTo(b),a.val(""))},groupEmailRemove:function(a){a.preventDefault();
$(a.target).parent().parent().remove()},groupForm:function(a){a.preventDefault();a=$(a.target).attr("href")+".json";OP.Util.makeRequest(a,{modal:"true",dynamic:"true"},function(a){if(200===a.code){var c=$("#modal"),a=f.modal("Create a group",a.result.markup,null);c.html(a).modal()}else opTheme.message.error("Could not load the form to create an album.")},"json","get");return!1},groupPost:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json",c=-1<b.search("create"),d=$('input[name="dynamic"]',
a).val(),e,f={name:$('input[name="name"]',a).val()};f.crumb=h.get();$(".group-email-add-click",a).trigger("click");e=[];$("span.group-email-queue",a).each(function(a,b){e.push($(b).html())});f.members=e.join(",");OP.Util.makeRequest(b,f,function(a){var b;200===a.code||201===a.code?d?opTheme.callback.groupPostDynamicCb(b,a.result):c?location.href="/manage/groups?m=group-created":opTheme.message.confirm("Group updated successfully."):opTheme.message.error("Could not update group.")})},groupPostDynamicCb:function(a,
...
OP.Util.on("click:album-delete",opTheme.callback.albumDelete);OP.Util.on("click:album-form",opTheme.callback.albumForm);OP.Util.on("click:batch-modal",opTheme.callback.batchModal);OP.Util.on("click:credential-view",opTheme.callback.credentialView);OP.Util.on("click:credential-view-dialogue-ok",opTheme.callback.credentialViewDialogueOk);OP.Util.on("click:credential-delete",opTheme.callback.credentialDelete);OP.Util.on("click:group-delete",opTheme.callback.groupDelete);OP.Util.on("click:group-email-add",
