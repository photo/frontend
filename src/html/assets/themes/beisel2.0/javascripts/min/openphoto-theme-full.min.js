...
$(this).attr("data-id");container=$(this).parent();container.hasClass("pinned")||OP.Batch.add(id)})},pluginView:function(a){a.preventDefault();var a=$(a.target).attr("href")+".json",b={crumb:h.get()},c=a.match(/\/plugin\/(.*)\/view.json$/);OP.Util.makeRequest("/plugin/"+c[1]+"/view.json",b,function(a){200===a.code?$("#modal").html(f.modal("Update "+c[1],a.result.markup,null)).modal():opTheme.message.error("Unable to load this plugin for editing.")},"json","get")},pluginStatusToggle:function(a){a.preventDefault();
var b=$(a.target),a=b.attr("href")+".json";OP.Util.makeRequest(a,{},function(a){var d=$(b).parent(),e=d.parent();200===a.code?($("div",e).removeClass("hide"),d.addClass("hide")):opTheme.message.error("Could not update the status of this plugin.")},"json","post");return!1},pluginUpdate:function(a){a.preventDefault();var a=$(a.target),b=a.attr("action")+".json";OP.Util.makeRequest(b,a.serializeArray(),function(a){200===a.code?opTheme.message.confirm("Your plugin was successfully updated."):opTheme.message.error("Could not update the status of this plugin.");
$("#modal").modal("hide")},"json","post");return!1},preloadPhoto:function(a){OP.Util.makeRequest("/photo/"+a.id+"/view.json",{returnSizes:a.sizes,generate:"true"},opTheme.callback.preloadPhotoCb,"json","get")},preloadPhotoCb:function(a){var b=a.result;if(200===a.code)for(i in b)b.hasOwnProperty(i)&&!0===/^path[0-9]/.test(i)&&k.fetchAndCache(b[i])},popup:function(a){a.preventDefault();var b=$(a.target),a=b.attr("data-width"),c=b.attr("data-height"),b=b.attr("href");window.open(b,"openphoto","toolbar=0,menubar=0,location=1,directories=0,channelmode=0,width="+
a+",height="+c)},searchByTags:function(a){a.preventDefault();var b=$(a.target),a=$("select[name=tags]",b).val().join(","),b=b.attr("action");0<a.length&&(location.href=0<b.search("/list")?b.replace("/list","")+"/tags-"+a+"/list":b+"?tags="+a);return!1},settings:function(){$("ul#settingsbar").slideToggle("medium");$("li#nav-signin").toggleClass("active");return!1},share:{facebook:function(a){a.preventDefault();var a=$(a.target),b={method:"feed",display:"popup"};b.link=a.attr("data-link");b.picture=
a.attr("data-picture");b.name=a.attr("data-name");b.description=a.attr("data-description");FB.ui(b,function(){window.close()})}},tagsInitialized:function(){var a=OP.Tag.getTags(),b="";if(null!==a&&0<a.length)for(i in a)b+='<option value="'+a[i]+'">'+a[i]+"</option>";$(".typeahead-tags").html(b).chosen()},uploadCompleteSuccess:function(a){$("form.upload").fadeOut("fast",function(){OP.Util.makeRequest("/photos/upload/confirm.json",a,opTheme.callback.uploadConfirm,"json","post")})},uploadConfirm:function(a){$("body.upload .upload-container").fadeOut("fast",
function(){$(".upload-confirm").fadeIn("fast")});$("body.upload .upload-confirm").html(a.result).show("fast")},uploaderReady:function(){$("form.upload");"object"===typeof OPU&&OPU.init();$("select.typeahead").chosen()},webhookDelete:function(a){a.preventDefault();var b=$(a.target),a=b.attr("href")+".json";OP.Util.makeRequest(a,{},function(a){204===a.code?(b.parent().remove(),opTheme.message.confirm("Credential successfully deleted.")):opTheme.message.error("Could not delete credential.")});return!1}},
init:{load:function(a){var b="state"in window.history&&null!==window.history.state,c=location.href;h.set(a);OP.Tag.init();l=location.pathname;History.Adapter.bind(window,"statechange",function(){var a=History.getState(),e=!b&&location.href==c;b=!0;e||g.render(a.data)});$(".dropdown-toggle").dropdown();$(".modal").on("shown",opTheme.callback.modalShown);"/"===location.pathname?opTheme.init.pages.front():"/manage/photos"===location.pathname?opTheme.init.pages.manage.photos():0===location.pathname.search(/^\/photos(.*)\/list/)?
...
a+") a").addClass("active")});$(".carousel.feed").carousel();$(".carousel.feed").carousel("pause")},manage:{photos:function(){var a=OP.Batch.collection.getAll(),b=OP.Batch.collection.getLength(),c=$(".pin"),d,e;0<b&&opTheme.ui.batchMessage();c.each(function(b,c){c=$(c);d=c.attr("class");e=d.match(/ photo-([a-z0-9]+)/);2==e.length&&void 0!==a[e[1]]&&c.addClass("revealed pinned")})}},photo:{init:function(){k.fetchAndCacheNextPrevious();OP.Util.fire("photo:viewed",{url:location.href})}},photos:{initData:"undefined"===
typeof initData?void 0:initData,filterOpts:"undefined"===typeof filterOpts?void 0:filterOpts,page:null,pageCount:0,pageLocation:window.location,maxMobilePageCount:5,end:!1,running:!1,init:function(){var a=opTheme.init.pages.photos;$(window).scroll(a.scrollCb);a.load()},scrollCb:function(){var a=opTheme.init.pages.photos;900>k.getDeviceWidth()||$(window).scrollTop()>$(document).height()-$(window).height()-200&&a.load()},load:function(){var a=opTheme.init.pages.photos;loc=location;if("undefined"===
typeof a.initData){if(!a.end&&!a.running){a.running=!0;if(null===a.page){var b=loc.href.match("page=([0-9]+)");null!==b?a.page=b[1]:(b=loc.pathname.match(/\/page-([0-9]+)/),null!==b&&(a.page=b[1]));null===a.page&&(a.page=1)}b=a.pageLocation.pathname+".json";params={};qs=a.pageLocation.search.replace("?","");if(0<qs.length){var c=qs.split("&"),d;for(i in c)c.hasOwnProperty(i)&&(d=c[i].split("="),2===d.length&&(params[d[0]]=d[1]))}params.returnSizes="960x180";params.page=a.page;a.pageCount>a.maxMobilePageCount&&
900>k.getDeviceWidth()?location.href=a.pageLocation.pathname+"?"+decodeURIComponent($.param(params)):$.getJSON(b,params,a.loadCb)}}else delete a.initData,a.page=1,a.loadCb({code:200,result:initData})},loadCb:function(a){var a=a.result,b=opTheme.init.pages.photos,c=$(".infobar"),d=$(".startdate",c),c=$(".enddate",c);parseInt(d.attr("data-time"));var e=parseInt(c.attr("data-time")),f=opTheme.ui,g;if(a[0].totalPages>=b.page){var h;for(g=0;g<a.length;g++)h=parseInt(a[g].dateTaken),h>e?(f.fadeAndSet(c,
phpjs.date("l F jS, Y",h)),c.attr("data-time",h),e=h):parseInt(a[g].dateTaken)<parseInt(d.attr("data-time"))&&(f.fadeAndSet(d,phpjs.date("l F jS, Y",h)),d.attr("data-time",h));Gallery.showImages($(".photo-grid-justify"),a);b.page++;b.pageCount++;b.running=!1}else $(".load-more").hide(),b.end=!0}},upload:function(){OP.Util.fire("upload:uploader-ready")}}},message:{append:function(a){var b=$(".message:first").clone(!1),c=$(".message:last");b.addClass("alert alert-info").html(a);c.after(b).slideDown();
b.delay(5E3).slideUp(function(){$(this).remove()})},confirm:function(a){opTheme.message.show(a,"confirm")},error:function(a){opTheme.message.show(a,"error")},show:function(a,b){opTheme.message.append(f.message(a,b))}},ui:{batchFormFields:{albums:function(){var a,b="";$.ajax({url:"/albums/list.json",data:{pageSize:"0"},async:!1,success:function(c){if(200===c.code){b+='<label>Select albums</label><select name="value" multiple>';for(i in c.result)c.result.hasOwnProperty(i)&&(a=c.result[i],b+='<option value="'+
a.id+'">'+a.name+"</option>");b+="</select>"}}});return b},empty:function(){return""},groups:function(){var a,b="";$.ajax({url:"/groups/list.json",async:!1,success:function(c){if(200===c.code){b+='<label>Select groups</label><select name="value" multiple>';for(i in c.result)c.result.hasOwnProperty(i)&&(a=c.result[i],b+='<option value="'+a.id+'">'+a.name+"</option>");b+="</select>"}}});return b},permission:function(){return'  <label class="radio">    <input type="radio" name="value" value="1" checked="checked">    Public  </label>  <label class="radio">    <input type="radio" name="value" value="0">     Private  </label>'},
